...
def load_gwy_tiles(tile_dir, center_crop_size=None):
    ...
    logger.info(f"Found {len(names)} .gwy files in {tile_dir}")
    ...
    for fname in names:
        ...
        try:
            ...
            logger.warning(f"Skip (read error): {fname}: {e}")
            if DEBUG:
        ...
        data_keys.sort()
        if not data_keys:
            logger.warning(f"No data fields in {fname}, available keys: {list(gwy.keys())}")
            if DEBUG:
        ...
        for di, key in enumerate(data_keys):
            ...
            if arr is None:
                ...
                if hasattr(df, '__array__'):
                    ...
                    logger.warning(f"Key {key} in {fname} has no data array; skipping")
                    if DEBUG:
                        print(f"Key {key} in {fname} has no data array; skipping")
                    continue
            arr = np.asarray(arr)
            if arr.ndim != 2:
                logger.warning(f"Key {key} in {fname} has ndim={arr.ndim} (expected 2); skipping")
                continue
            ...
            logger.debug(f"Processing {fname}:{key} - shape: {arr.shape}, xres: {xres}, yres: {yres}")
            ...
            if ch_name not in mats_per_channel:
                ...
                logger.info(f"Added channel: {ch_name}")
    ...
    if not mats_per_channel:
        logger.error(f"No usable data fields found in GWY files. Processed {len(names)} files from {tile_dir}")
...
def interactive_tiff_view(channel_matrices, path_matrix, height, width, output_dir, base_filename, channel_metadata=None, tile_names=None):
    ...
    def open_matplotlib_view():
        ...
        unit = channel_unit(selected) or ''
        cbar.set_label(unit)

        def _cbar_fmt(x, pos):
            try:
                mag = abs(x)
                if mag != 0 and (mag >= 1e3 or mag < 1e-2):
                    s = f"{x:.3e}"
                elif mag < 1:
                    s = f"{x:.3f}"
                else:
            ...
            if unit in ('deg', '°'):
                return f"{s}°"
            if unit.lower() in ('nm', 'nanometer', 'nanometre'):
                return f"{s} nm"
            if unit and unit not in ('a.u.', 'au', ''):
                return f"{s} {unit}"
            return s
