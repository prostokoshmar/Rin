...
def _make_sift_instance(nfeatures=0, contrastThreshold=0.04, edgeThreshold=10, sigma=1.6):
    
    try:
        sift = SIFT(sigma=float(sigma), contrast_threshold=float(contrastThreshold), edge_threshold=float(edgeThreshold))
        return sift
    except TypeError:
        
        logging.info("SIFT: constructor did not accept custom kwargs; using default SIFT()")
        return SIFT()

def _match_descriptors_ratio_and_crosscheck(d1, d2, ratio=0.75):
    
    if d1 is None or d2 is None or len(d1) == 0 or len(d2) == 0:
        return np.empty((0, 2), dtype=int)

    
    nn12 = NearestNeighbors(n_neighbors=2, algorithm='auto').fit(d2)
    dists12, idxs12 = nn12.kneighbors(d1, return_distance=True)

    
    good12 = {}
    for i in range(d1.shape[0]):
        if dists12[i, 0] < 1e-12:
            
            if dists12[i, 1] < 1e-12:
                good12[i] = int(idxs12[i, 0])
        else:
            if dists12[i, 0] < ratio * dists12[i, 1]:
                good12[i] = int(idxs12[i, 0])

    
    nn21 = NearestNeighbors(n_neighbors=2, algorithm='auto').fit(d1)
    dists21, idxs21 = nn21.kneighbors(d2, return_distance=True)
    good21 = {}
    for j in range(d2.shape[0]):
        if dists21[j, 0] < 1e-12:
            if dists21[j, 1] < 1e-12:
                good21[j] = int(idxs21[j, 0])
        else:
            if dists21[j, 0] < ratio * dists21[j, 1]:
                good21[j] = int(idxs21[j, 0])

    
    pairs = []
    for i, j in good12.items():
        if j in good21 and good21[j] == i:
            pairs.append((i, j))
...
def match_features_sift(img1, img2, method="sift", nfeatures=0, contrastThreshold=0.04, edgeThreshold=10, sigma=1.6, ratio_test=0.75):
    
    logging.info(f"Debug SIFT params: nfeatures={nfeatures}, contrastThreshold={contrastThreshold}, edgeThreshold={edgeThreshold}, sigma={sigma}, ratio_test={ratio_test}")
    sift1 = _make_sift_instance(nfeatures=nfeatures, contrastThreshold=contrastThreshold, edgeThreshold=edgeThreshold, sigma=sigma)
    sift2 = _make_sift_instance(nfeatures=nfeatures, contrastThreshold=contrastThreshold, edgeThreshold=edgeThreshold, sigma=sigma)
    sift1.detect_and_extract(img1)
    sift2.detect_and_extract(img2)

    k1, d1 = sift1.keypoints, sift1.descriptors
    k2, d2 = sift2.keypoints, sift2.descriptors

    
    if nfeatures and nfeatures > 0:
        if d1 is not None and len(d1) > nfeatures:
            k1 = k1[:int(nfeatures)].copy()
            d1 = d1[:int(nfeatures)].copy()
        if d2 is not None and len(d2) > nfeatures:
            k2 = k2[:int(nfeatures)].copy()
            d2 = d2[:int(nfeatures)].copy()

    
    try:
        matches = _match_descriptors_ratio_and_crosscheck(d1, d2, ratio=ratio_test)
    except Exception as e:
        logging.info(f"SIFT matching with ratio test failed, falling back to match_descriptors: {e}")
...
def match_features_dense_sift(img1, img2, method="dense_sift", step=8, size=64, nfeatures=0, contrastThreshold=0.04, edgeThreshold=10, sigma=1.6, ratio_test=0.75):
    
    logging.info(f"Debug DENSE_SIFT params: step={step}, size={size}, nfeatures={nfeatures}, contrastThreshold={contrastThreshold}, edgeThreshold={edgeThreshold}, sigma={sigma}, ratio_test={ratio_test}")
    ...
    if d1 is None or d2 is None:
        return None, np.array(k1), np.array(k2), np.empty((0, 2), dtype=int)
...
def match_features_orb(img1, img2, method="orb", nfeatures=2000, scaleFactor=1.2, nlevels=8, fastThreshold=15, ratio_test=0.68):
    ...
    try:
        a1 = img1 if img1.dtype.kind == 'f' else img_as_float(img1)
        a2 = img2 if img2.dtype.kind == 'f' else img_as_float(img2)
    except Exception:
        a1 = img_as_float(img1)
        a2 = img_as_float(img2)

    
    orb1 = ORB(n_keypoints=max(50, int(nfeatures)))
    orb2 = ORB(n_keypoints=max(50, int(nfeatures)))

    try:
        orb1.detect_and_extract(a1)
        orb2.detect_and_extract(a2)
    except Exception as e:
        logging.info(f"Debug ORB: detect_and_extract failed: {e}")
        return None, np.empty((0, 2)), np.empty((0, 2)), np.empty((0, 2), dtype=int)

    k1 = np.array(orb1.keypoints) if orb1.keypoints is not None else np.empty((0, 2))
    k2 = np.array(orb2.keypoints) if orb2.keypoints is not None else np.empty((0, 2))
    d1 = orb1.descriptors
    d2 = orb2.descriptors

    if d1 is None or d2 is None or len(k1) < min_matches or len(k2) < min_matches:
        logging.info(f"Debug ORB: Not enough descriptors (d1={None if d1 is None else len(d1)}, d2={None if d2 is None else len(d2)}) or keypoints (k1={len(k1)}, k2={len(k2)})")
        return None, k1, k2, np.empty((0, 2), dtype=int)

    
    try:
        matches = match_descriptors(d1, d2, metric='hamming', cross_check=True)
    except Exception:
        matches = np.empty((0, 2), dtype=int)

    if matches.size == 0 or len(matches) < min_matches:
        return None, k1, k2, np.empty((0, 2), dtype=int)
...
def apply_ransac_to_matches(k1, k2, matches, model='affine', thresh=5.0, min_inliers=4):
    ...
    src_pts = np.array([k1[int(m[0])][::-1] for m in matches], dtype=np.float64)
    dst_pts = np.array([k2[int(m[1])][::-1] for m in matches], dtype=np.float64)
    ...
    try:
        model_robust, inliers = ransac((src_pts, dst_pts), Model, min_samples=min_samples, residual_threshold=thresh, max_trials=1000)
    except Exception:
    ...
    try:
        ...
        M[:model_robust.params.shape[0], :model_robust.params.shape[1]] = model_robust.params
    except Exception:
        
        try:
            M = np.array(model_robust.params)
        except Exception:
            M = None
...
if __name__ == "__main__":
    ...
    TILE_DIR = "/Volumes/T7/last/NEWDATA/0V/Height_Backward"  # Replace with your folder path
    MATCH_VIS_DIR = os.path.join(TILE_DIR, "match_vis_skimage")
    matching_ratio = 0.1 #Скільки відсотків правильних (зелених) ліній
    matching_angle = 10 # максимальний кут відхилення в градусах
    min_matches = 4  # Зменшено для тестування
    len_gap = 20  # Максимальна відстань від центроїда кластера для валідних ліній
    selected_method = "sift"  # Вибираємо метод для обчислення середніх значень
